...
export const CATALOG = Object.freeze([
  ...
  entry({
    ...
    outputCapability: 235_929,
    vendorInput: ['text', 'image'],
    effectiveImage: true,
    category: 'coding',
    aliases: ['kimi k2.7 code', 'kimi-k2.7-code', 'kimi k2.7-code'],
    vendorReasoningNote: 'OpenRouter moonshotai/kimi-k2.7-code on 2026-09-25: context 262144, max completion 235929, inputs text and image. Its parameter list includes reasoning but not reasoning_effort, so no effort list is offered. api.cline.bot image input was not measured.',
  }),
  entry({
    ...
    vendorReasoningNote: 'OpenRouter moonshotai/kimi-k2.6 on 2026-09-25: context 262144, max completion 235929, inputs text and image. reasoning is listed, reasoning_effort is not, so no effort list is offered. api.cline.bot image input was not measured.',
  }),
  entry({
  ...
  entry({
    ...
    vendorReasoningNote: 'OpenRouter qwen/qwen3.8-max-0902 and qwen/qwen3.8-max-prime on 2026-09-25 both list context 1000000, max completion 131072, and text/image/video input. reasoning_effort is accepted, but the effort names are not published, so no list is offered. api.cline.bot image input was not measured.',
  }),
  entry({
    ...
    vendorReasoningNote: 'OpenRouter qwen/qwen3.7-max on 2026-09-25: context 1000000, max completion 131072, text input. reasoning is listed without reasoning_effort, so no effort list is offered.',
  }),
  entry({
    ...
    vendorReasoningNote: 'OpenRouter qwen/qwen3.7-plus on 2026-09-25: context 1000000, max completion 131072, text and image input. A 256K figure is a price break, not this context window. reasoning is listed without reasoning_effort, so no effort list is offered. api.cline.bot image input was not measured.',
  }),
  entry({
  ...
  entry({
    ...
    contextWindow: 1_048_576,
    outputCapability: 943_718,
    vendorInput: ['text', 'image'],
    effectiveImage: true,
    category: 'general',
    aliases: ['muse spark 1.3 contributor', 'muse-spark-1.3-contributor'],
    vendorReasoningNote: 'OpenRouter meta/muse-spark-1.3-contributor on 2026-09-25: context 1048576, max completion 943718, inputs text, image, video, file, and audio. Meta\'s reasoning guide lists minimal, low, medium, high, and xhigh; max is standard-tier only and is omitted here. api.cline.bot image input and those efforts were not live-called. The smoke test still sends reasoning_effort low.',
    optionalReasoning: { minimal: 'minimal', low: 'low', medium: 'medium', high: 'high', xhigh: 'xhigh' },
    optionalCompat: EFFORT_MAX_TOKENS_COMPAT,
  }),
])
